...
static void update_layer_state(struct temp_layer_state *state, bool activate) {
    ...
    if (activate) {
        zmk_flag_activate(state->toggle_layer, false);
        LOG_DBG("Layer %d activated", state->toggle_layer);
    } else {
        zmk_flag_deactivate(state->toggle_layer, false);
...
static void layer_action_work_cb(struct k_work *work) {
    ...
    while (k_msgq_get(&temp_layer_action_msgq, &action, K_MSEC(10)) >= 0) {
        if (!action.activate) {
            if (zmk_flag_is_active(action.layer)) {
...
static int handle_flag_state_changed(const struct device *dev, const zmk_event_t *eh) {
    ...
    if (!zmk_flag_is_active(zmk_keymap_layer_index_to_id(data->state.toggle_layer))) {
...
ZMK_SUBSCRIPTION(processor_temp_layer, zmk_flag_state_changed);
